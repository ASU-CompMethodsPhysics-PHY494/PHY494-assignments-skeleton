...
\item \label{li:error} Analyze the error in the total energy $E$ by
  ...
  \end{enumi}
\item \BONUS Perform the MD in \ref{li:euler} with the \emph{velocity
    Verlet} algorithm (create a function
  \mintinline{python}{dimer_md_vv()}) and perform the error analysis
  as in \ref{li:error}. \bonus{4}
\end{enuma}
